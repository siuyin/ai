...
// SentenceOld is a proposition (as in Propositional logic).
type SentenceOld struct {
	Prop string // Prop for proposition eg. It is raining.
	Val  bool   // Is the proposition true or false?
}

func (s SentenceOld) And(t SentenceOld) SentenceOld {
	ns := SentenceOld{}
	ns.Prop = fmt.Sprintf("(%s AND %s)", s, t)
	ns.Val = s.Val && t.Val
	return ns
}
func (s SentenceOld) String() string {
	if !s.Val {
		return fmt.Sprintf("(NOT %s)", s.Prop)
	}
	return s.Prop
}
func (s SentenceOld) Or(t SentenceOld) SentenceOld {
	ns := SentenceOld{}
	ns.Prop = fmt.Sprintf("(%s OR %s)", s, t)
	ns.Val = s.Val || t.Val
	return ns
}
func (s SentenceOld) Implies(t SentenceOld) SentenceOld {
	ns := SentenceOld{}
	ns.Prop = fmt.Sprintf("(%s IMPLIES %s)", s, t)
	ns.Val = !s.Val || t.Val // see https://en.wikipedia.org/wiki/Material_implication_(rule_of_inference)
	return ns
}
func (s SentenceOld) Not() SentenceOld {
	ns := SentenceOld{}
	ns.Prop = fmt.Sprintf("(NOT %s)", s)
	ns.Val = !s.Val
	return ns
}
func (s SentenceOld) Biconditional(t SentenceOld) SentenceOld {
	ns := SentenceOld{}
	ns.Prop = fmt.Sprintf("(%s BICONDITIONAL %s)", s, t)
	// (s -> t) && (t -> s) is equivalent to (!s || t) && (!t || s)
	ns.Val = (!s.Val || t.Val) && (!t.Val || s.Val)
	return ns
}
